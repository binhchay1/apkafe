...
class Handler
{
     ...
     * @var bool
     */
    private static $enableShutdownHandler = \true;
    /**
    ...
    {
        if (!$client instanceof \Bugsnag\Client) {
            $client = \Bugsnag\Client::make($client);
        }
        // @phpstan-ignore-next-line
        $handler = new static($client);
        $handler->registerBugsnagHandlers(\true);
        return $handler;
    }
    /**
     ...
     * @return void
     */
    public function __construct(\Bugsnag\Client $client)
    {
        $this->client = $client;
    }
    /**
    ...
    {
        $report = \Bugsnag\Report::fromPHPThrowable($this->client->getConfig(), $throwable);
        $report->setSeverity('error');
        $report->setUnhandled(\true);
    ...
    public function errorHandler($errno, $errstr, $errfile = '', $errline = 0)
    {
        if (!$this->client->getConfig()->shouldIgnoreErrorCode($errno)) {
            $report = \Bugsnag\Report::fromPHPError($this->client->getConfig(), $errno, $errstr, $errfile, $errline, \false);
            $report->setUnhandled(\true);
            $report->setSeverityReason(['type' => 'unhandledError', 'attributes' => ['errorType' => \Bugsnag\ErrorTypes::getName($errno)]]);
            $this->client->notify($report);
        }
    ...
    public function shutdownHandler()
    {
        ...
        if (!\is_null($lastError) && \Bugsnag\ErrorTypes::isFatal($lastError['type']) && !$this->client->getConfig()->shouldIgnoreErrorCode($lastError['type'])) {
            $report = \Bugsnag\Report::fromPHPError($this->client->getConfig(), $lastError['type'], $lastError['message'], $lastError['file'], $lastError['line'], \true);
            $report->setSeverity('error');
            $report->setUnhandled(\true);
            $report->setSeverityReason(['type' => 'unhandledException']);
            $this->client->notify($report);
