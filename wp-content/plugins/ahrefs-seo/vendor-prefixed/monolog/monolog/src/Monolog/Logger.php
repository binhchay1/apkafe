...
use ahrefs\AhrefsSeo_Vendor\Monolog\Handler\HandlerInterface;
use ahrefs\AhrefsSeo_Vendor\Monolog\Handler\StreamHandler;
use ahrefs\AhrefsSeo_Vendor\Psr\Log\LoggerInterface;
use ahrefs\AhrefsSeo_Vendor\Psr\Log\InvalidArgumentException;
use Exception;
/**
 * Monolog log channel
 *
 ...
 */
class Logger implements \ahrefs\AhrefsSeo_Vendor\Psr\Log\LoggerInterface, \ahrefs\AhrefsSeo_Vendor\Monolog\ResettableInterface
{
    /**
     ...
     * @return $this
     */
    public function pushHandler(\ahrefs\AhrefsSeo_Vendor\Monolog\Handler\HandlerInterface $handler)
    {
    ...
    {
        if (!$this->handlers) {
            $this->pushHandler(new \ahrefs\AhrefsSeo_Vendor\Monolog\Handler\StreamHandler('php://stderr', static::DEBUG));
        }
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \ahrefs\AhrefsSeo_Vendor\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \ahrefs\AhrefsSeo_Vendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName($level)
    {
        if (!isset(static::$levels[$level])) {
            throw new \ahrefs\AhrefsSeo_Vendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
