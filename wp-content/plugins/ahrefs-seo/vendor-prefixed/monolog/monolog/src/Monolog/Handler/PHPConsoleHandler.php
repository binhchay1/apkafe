...
use ahrefs\AhrefsSeo_Vendor\PhpConsole\Connector;
use ahrefs\AhrefsSeo_Vendor\PhpConsole\Handler;
use ahrefs\AhrefsSeo_Vendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \ahrefs\AhrefsSeo_Vendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \ahrefs\AhrefsSeo_Vendor\PhpConsole\Connector $connector = null, $level = \ahrefs\AhrefsSeo_Vendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('ahrefs\\AhrefsSeo_Vendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\ahrefs\AhrefsSeo_Vendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \ahrefs\AhrefsSeo_Vendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \ahrefs\AhrefsSeo_Vendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\ahrefs\AhrefsSeo_Vendor\PhpConsole\Helper::isRegistered()) {
            \ahrefs\AhrefsSeo_Vendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \ahrefs\AhrefsSeo_Vendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \ahrefs\AhrefsSeo_Vendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \ahrefs\AhrefsSeo_Vendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \ahrefs\AhrefsSeo_Vendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
