 ...
 */
namespace ahrefs\AhrefsSeo_Vendor\Google\Auth\Credentials;

use ahrefs\AhrefsSeo_Vendor\Google\Auth\CredentialsLoader;
use ahrefs\AhrefsSeo_Vendor\Google\Auth\GetQuotaProjectInterface;
...
use ahrefs\AhrefsSeo_Vendor\Google\Auth\Iam;
use ahrefs\AhrefsSeo_Vendor\Google\Auth\ProjectIdProviderInterface;
use ahrefs\AhrefsSeo_Vendor\Google\Auth\SignBlobInterface;
use ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Exception\ClientException;
...
use ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Exception\ServerException;
 ...
 */
class GCECredentials extends \ahrefs\AhrefsSeo_Vendor\Google\Auth\CredentialsLoader implements \ahrefs\AhrefsSeo_Vendor\Google\Auth\SignBlobInterface, \ahrefs\AhrefsSeo_Vendor\Google\Auth\ProjectIdProviderInterface, \ahrefs\AhrefsSeo_Vendor\Google\Auth\GetQuotaProjectInterface
{
    // phpcs:disable
     ...
     */
    public function __construct(\ahrefs\AhrefsSeo_Vendor\Google\Auth\Iam $iam = null, $scope = null, $targetAudience = null, $quotaProject = null, $serviceAccountIdentity = null)
    {
        $this->iam = $iam;
    ...
    {
        $httpHandler = $httpHandler ?: \ahrefs\AhrefsSeo_Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build(\ahrefs\AhrefsSeo_Vendor\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        ...
        for ($i = 1; $i <= self::MAX_COMPUTE_PING_TRIES; $i++) {
            try {
                ...
                // the metadata resolution was particularly slow. The latter case is
                // "unlikely".
                $resp = $httpHandler(new \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Psr7\Request('GET', $checkUri, [self::FLAVOR_HEADER => 'Google']), ['timeout' => self::COMPUTE_PING_CONNECTION_TIMEOUT_S]);
                return $resp->getHeaderLine(self::FLAVOR_HEADER) == 'Google';
            } catch (\ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Exception\ClientException $e) {
            } catch (\ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Exception\ServerException $e) {
    ...
    {
        $httpHandler = \ahrefs\AhrefsSeo_Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build(\ahrefs\AhrefsSeo_Vendor\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        ...
        $signer = $this->iam ?: new \ahrefs\AhrefsSeo_Vendor\Google\Auth\Iam($httpHandler);
    ...
    {
        $resp = $httpHandler(new \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Psr7\Request('GET', $uri, [self::FLAVOR_HEADER => 'Google']));
