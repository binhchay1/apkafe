...
use ahrefs\AhrefsSeo_Vendor\Google\Auth\ServiceAccountSignerTrait;
 ...
 */
class ServiceAccountCredentials extends \ahrefs\AhrefsSeo_Vendor\Google\Auth\CredentialsLoader implements \ahrefs\AhrefsSeo_Vendor\Google\Auth\GetQuotaProjectInterface, \ahrefs\AhrefsSeo_Vendor\Google\Auth\SignBlobInterface, \ahrefs\AhrefsSeo_Vendor\Google\Auth\ProjectIdProviderInterface
{
    ...
    public function __construct($scope, $jsonKey, $sub = null, $targetAudience = null)
    {
        ...
        $this->auth = new \ahrefs\AhrefsSeo_Vendor\Google\Auth\OAuth2(['audience' => self::TOKEN_CREDENTIAL_URI, 'issuer' => $jsonKey['client_email'], 'scope' => $scope, 'signingAlgorithm' => 'RS256', 'signingKey' => $jsonKey['private_key'], 'sub' => $sub, 'tokenCredentialUri' => self::TOKEN_CREDENTIAL_URI, 'additionalClaims' => $additionalClaims]);
    ...
    private function createJwtAccessCredentials()
    {
        if (!$this->jwtAccessCredentials) {
            ...
            $this->jwtAccessCredentials = new \ahrefs\AhrefsSeo_Vendor\Google\Auth\Credentials\ServiceAccountJwtAccessCredentials($credJson, $this->auth->getScope());
        }
