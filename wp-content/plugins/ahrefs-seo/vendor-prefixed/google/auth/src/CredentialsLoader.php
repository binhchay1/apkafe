...
use ahrefs\AhrefsSeo_Vendor\Google\Auth\Credentials\InsecureCredentials;
 ...
 */
abstract class CredentialsLoader implements \ahrefs\AhrefsSeo_Vendor\Google\Auth\FetchAuthTokenInterface, \ahrefs\AhrefsSeo_Vendor\Google\Auth\UpdateMetadataInterface
{
    ...
    private static function getGuzzleMajorVersion()
    {
        if (\defined('ahrefs\\AhrefsSeo_Vendor\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
            return \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\ClientInterface::MAJOR_VERSION;
        }
        if (\defined('ahrefs\\AhrefsSeo_Vendor\\GuzzleHttp\\ClientInterface::VERSION')) {
            return (int) \substr(\ahrefs\AhrefsSeo_Vendor\GuzzleHttp\ClientInterface::VERSION, 0, 1);
        }
    ...
    public static function makeCredentials($scope, array $jsonKey, $defaultScope = null)
    {
        ...
        if ($jsonKey['type'] == 'service_account') {
            ...
            return new \ahrefs\AhrefsSeo_Vendor\Google\Auth\Credentials\ServiceAccountCredentials($scope, $jsonKey);
        }
        if ($jsonKey['type'] == 'authorized_user') {
            $anyScope = $scope ?: $defaultScope;
            return new \ahrefs\AhrefsSeo_Vendor\Google\Auth\Credentials\UserRefreshCredentials($anyScope, $jsonKey);
        }
     ...
     */
    public static function makeHttpClient(\ahrefs\AhrefsSeo_Vendor\Google\Auth\FetchAuthTokenInterface $fetcher, array $httpClientOptions = [], callable $httpHandler = null, callable $tokenCallback = null)
    {
        if (self::getGuzzleMajorVersion() === 5) {
            $client = new \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Client($httpClientOptions);
            $client->setDefaultOption('auth', 'google_auth');
            $subscriber = new \ahrefs\AhrefsSeo_Vendor\Google\Auth\Subscriber\AuthTokenSubscriber($fetcher, $httpHandler, $tokenCallback);
            $client->getEmitter()->attach($subscriber);
            return $client;
        }
        $middleware = new \ahrefs\AhrefsSeo_Vendor\Google\Auth\Middleware\AuthTokenMiddleware($fetcher, $httpHandler, $tokenCallback);
        $stack = \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\HandlerStack::create();
        $stack->push($middleware);
        return new \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Client(['handler' => $stack, 'auth' => 'google_auth'] + $httpClientOptions);
    }
    /**
    ...
    {
        return new \ahrefs\AhrefsSeo_Vendor\Google\Auth\Credentials\InsecureCredentials();
    }
    /**
