...
use DateTime;
use Exception;
use ahrefs\AhrefsSeo_Vendor\Firebase\JWT\ExpiredException;
use ahrefs\AhrefsSeo_Vendor\Firebase\JWT\JWT;
...
use ahrefs\AhrefsSeo_Vendor\SimpleJWT\InvalidTokenException;
use ahrefs\AhrefsSeo_Vendor\SimpleJWT\JWT as SimpleJWT;
use ahrefs\AhrefsSeo_Vendor\SimpleJWT\Keys\KeyFactory;
use ahrefs\AhrefsSeo_Vendor\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
     ...
     */
    public function __construct(callable $httpHandler = null, \ahrefs\AhrefsSeo_Vendor\Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        $this->httpHandler = $httpHandler ?: \ahrefs\AhrefsSeo_Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build(\ahrefs\AhrefsSeo_Vendor\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        $this->cache = $cache ?: new \ahrefs\AhrefsSeo_Vendor\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
    ...
    public function verify($token, array $options = [])
    {
        ...
        try {
            ...
            // firebase/php-jwt 3+
        } catch (\ahrefs\AhrefsSeo_Vendor\SignatureInvalidException $e) {
            // firebase/php-jwt 2
        } catch (\ahrefs\AhrefsSeo_Vendor\SimpleJWT\InvalidTokenException $e) {
            // simplejwt
        } catch (\ahrefs\AhrefsSeo_Vendor\Google\Auth\DomainException $e) {
    ...
    {
        $this->checkSimpleJwt();
        $jwkset = new \ahrefs\AhrefsSeo_Vendor\SimpleJWT\Keys\KeySet();
        foreach ($certs as $cert) {
            $jwkset->add(\ahrefs\AhrefsSeo_Vendor\SimpleJWT\Keys\KeyFactory::create($cert, 'php'));
        }
    ...
    private function verifyRs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        foreach ($certs as $cert) {
            ...
            $rsa->loadKey(['n' => new \ahrefs\AhrefsSeo_Vendor\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['n']]), 256), 'e' => new \ahrefs\AhrefsSeo_Vendor\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['e']]), 256)]);
    ...
    public function revoke($token, array $options = [])
    {
        ...
        $body = \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Psr7\Utils::streamFor(\http_build_query(['token' => $token]));
        $request = new \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Psr7\Request('POST', self::OAUTH2_REVOKE_URI, ['Cache-Control' => 'no-store', 'Content-Type' => 'application/x-www-form-urlencoded'], $body);
    ...
    private function retrieveCertsFromLocation($url, array $options = [])
    {
        ...
        $httpHandler = $this->httpHandler;
        $response = $httpHandler(new \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Psr7\Request('GET', $url), $options);
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists('ahrefs\\AhrefsSeo_Vendor\\SimpleJWT\\JWT')) {
    ...
    private function setPhpsecConstants()
    {
        if (\filter_var(\getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            ...
            if (!\defined('ahrefs\\AhrefsSeo_Vendor\\CRYPT_RSA_MODE')) {
                \define('ahrefs\\AhrefsSeo_Vendor\\CRYPT_RSA_MODE', \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\RSA::MODE_OPENSSL);
            }
        }
    }
    /**
    ...
    {
        $class = 'ahrefs\\AhrefsSeo_Vendor\\Firebase\\JWT\\JWT';
