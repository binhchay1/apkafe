...
use ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Psr7\Query;
...
class SimpleMiddleware
{
    ...
    public function __invoke(callable $handler)
    {
        return function (\ahrefs\AhrefsSeo_Vendor\Psr\Http\Message\RequestInterface $request, array $options) use($handler) {
            ...
            $query = \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Psr7\Query::parse($request->getUri()->getQuery());
            $params = \array_merge($query, $this->config);
            $uri = $request->getUri()->withQuery(\ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Psr7\Query::build($params));
