...
class Ahrefs_Seo_Db_Helper {
	 ...
	 */
	public static function get_updated_post_tax_strings( array $post_tax_strings_with_ver, int $snapshot_id ) : array {
		global $wpdb;
		$ids                   = [];
		$updated_ids           = [];
		...
		foreach ( (array) $data as $row ) {
			$found_versions [ $row[0] ] = intval( $row[1] ); // index is "post_id|taxonomy".
		}
	 ...
	 */
	public static function content_data_get_clear_months( int $snapshot_id, string $search_string = '' ) : array {
		global $wpdb;
	 ...
	 * @return void
	 */
	public static function set_last_well_date( Post_Tax $post_tax, bool $set_to_current_date = true ) : void {
		global $wpdb;
		$wpdb->update(
			$wpdb->ahrefs_content,
			[ 'last_well_date' => $set_to_current_date ? date( 'Y-m-d' ) : null ],
	 ...
	 */
	public static function load_additional_data_from_history( Post_Tax $post_tax, string $field ) : ?array {
		global $wpdb;
		$data = null;
		switch ( $field ) {
