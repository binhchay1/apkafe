...
if ( ! function_exists( 'print_share_config' ) ) {
	 ...
	 * @return void
	 */
	function print_share_config( bool $is_wizard ) : void {
		if ( $is_wizard ) {
...
if ( $locals['no_ga'] && $locals['no_gsc'] ) {
	...
	<div class="ahrefs-content-tip tip-notice">
		...
		<div class="text"><?php esc_html_e( 'There are no Google Analytics & Search Console accounts connected to the Google profile. Please create GA and GSC accounts for your website or connect another Google profile.', 'ahrefs-seo' ); ?>
			<br>
			<?php Ahrefs_Seo::get()->get_view()->learn_more_link( 'https://help.ahrefs.com/en/articles/4666920-how-do-i-connect-google-analytics-search-console-to-the-plugin', __( 'How do I connect Google accounts?', 'ahrefs-seo' ) ); ?>
		</div>
		<?php print_share_config( $is_wizard ); ?>
	</div>
	<?php
} elseif ( $locals['no_ga'] ) {
