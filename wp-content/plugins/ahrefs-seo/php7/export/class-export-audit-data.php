...
class Export_Audit_Data {
	...
	public function get_export_url( bool $export_csv = false, string $tab = '' ) : string {
		return add_query_arg(
			...
			$export_csv ? Links::content_audit( $tab ) : Links::settings( Ahrefs_Seo_Screen_Settings::TAB_DATA ) // fallback to the "My audit data" settings page on zip export.
		);
	}
					...
					foreach ( $columns as $column_id => $title ) {
						switch ( $column_id ) {
							...
							case 'keyword':
								...
								$fields[] = function_exists( 'mb_strtolower' )
									? mb_strtolower( trim( str_replace( '✓', '', sanitize_text_field( (string) ob_get_clean() ) ) ) )
									: strtolower( trim( str_replace( '✓', '', sanitize_text_field( (string) ob_get_clean() ) ) ) );
								break;
							case 'categories':
								$value = '';
