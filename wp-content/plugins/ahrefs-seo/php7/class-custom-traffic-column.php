			...
			if ( ! $post_tax->is_tax_or_published() || is_null( $traffic ) || ( is_null( $traffic['organic_month'] ) && is_null( $traffic['organic_total'] ) ) || ( $traffic['organic_month'] < 0 ) || ( $traffic['organic_total'] < 0 ) ) {
				...
				<div class="ahrefs-traffic-wrapper">
					<span class="ahrefs_traffic_month"><span
								class="ahrefs_value"><?php echo esc_html( $traffic['organic_month'] ?? '—' ); ?></span> <?php esc_html_e( '/month', 'ahrefs-seo' ); ?></span>
					<span class="ahrefs_traffic_total"><?php esc_html_e( 'Total:', 'ahrefs-seo' ); ?> <span
								class="ahrefs_value"><?php echo esc_html( $traffic['organic_total'] ?? '—' ); ?></span></span>
				</div>
				<?php
			}
