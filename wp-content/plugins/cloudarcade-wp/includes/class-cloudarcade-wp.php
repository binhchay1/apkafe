...
class Cloudarcade_Wp {
	...
	private function load_dependencies() {
    	...
    	require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-cloudarcade-wp-widgets.php';
	...
	public static function setup_second_db() {
        global $cloudarcade_db;
        if($cloudarcade_db != null){
        	return;
        }
		$settings = new cloudarcadeSettingsAPI();
		$val = $settings->get_d();
		if($val){
			$cloudarcade_db = new wpdb($val['u'], $val['p'], $val['d'], $val['h']);
			// Check the connection
			if ($cloudarcade_db->last_error) {
