		...
		if ( $this->model->is_translated_taxonomy( $taxonomy ) ) {
			...
			if ( isset( $traces[ $n ]['function'], $traces[ $n ]['args'][0] ) && 'get_term_by' === $traces[ $n ]['function'] ) {
				...
				if ( in_array( $field, array( 'slug', 'name' ) ) ) {
					...
					$where  = $wpdb->prepare( "WHERE tt.taxonomy = %s AND t.{$field} = %s", $taxonomy, $term->$field ); // PHPCS:ignore WordPress.DB.PreparedSQL.NotPrepared
					$where .= $this->model->term->where_clause( $this->curlang );
					$term   = $wpdb->get_row( "SELECT t.*, tt.* FROM {$wpdb->terms} AS t {$join} {$where} LIMIT 1" ); // PHPCS:ignore WordPress.DB.PreparedSQL.NotPrepared
