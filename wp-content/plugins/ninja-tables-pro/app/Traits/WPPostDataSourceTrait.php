...
trait WPPostDataSourceTrait
{
    ...
    public function buildWPQuery($data)
    {
        ...
        $currentUser  = Arr::get($queryExtra, 'current_user_posts');

        $args = array(
            'post_type'      => $postTypes,
            'posts_per_page' => $perPage,
            'offset'         => $offset,
            'order'          => Arr::get($queryExtra, 'order_by'),
            'orderby'        => Arr::get($queryExtra, 'order_by_column'),
            'author'         => $currentUser === 'true' ? get_current_user_id() : false,
        );
    ...
    private function getFeaturedImage($post, $column)
    {
        $featuredImageUrl = get_the_post_thumbnail_url($post,
            $this->get($column, 'wp_post_custom_data_value', 'thumbnail'));
        if ( ! $featuredImageUrl) {
            return '';
        }
