...
use LassoVendor\GuzzleHttp\Exception\TransferException;
...
use LassoVendor\Kevinrob\GuzzleCache\Strategy\CacheStrategyInterface;
use LassoVendor\Kevinrob\GuzzleCache\Strategy\PrivateCacheStrategy;
...
class CacheMiddleware
{
    ...
    {
        \LassoVendor\GuzzleHttp\Promise\inspect_all($this->waitingRevalidate);
    }
    /**
    ...
    protected static function addToCache(CacheStrategyInterface $cache, RequestInterface $request, ResponseInterface $response, $update = \false)
    {
        ...
        if (!$response->getBody()->isSeekable()) {
            $response = $response->withBody(\LassoVendor\GuzzleHttp\Psr7\Utils::streamFor($response->getBody()->getContents()));
        }
        if ($update) {
