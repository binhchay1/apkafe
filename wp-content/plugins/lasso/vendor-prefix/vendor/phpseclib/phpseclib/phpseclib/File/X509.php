 ...
 */
namespace LassoVendor\phpseclib3\File;

use LassoVendor\phpseclib3\Common\Functions\Strings;
use LassoVendor\phpseclib3\Crypt\Common\PrivateKey;
use LassoVendor\phpseclib3\Crypt\Common\PublicKey;
use LassoVendor\phpseclib3\Crypt\DSA;
use LassoVendor\phpseclib3\Crypt\EC;
use LassoVendor\phpseclib3\Crypt\Hash;
use LassoVendor\phpseclib3\Crypt\PublicKeyLoader;
use LassoVendor\phpseclib3\Crypt\Random;
use LassoVendor\phpseclib3\Crypt\RSA;
use LassoVendor\phpseclib3\Crypt\RSA\Formats\Keys\PSS;
use LassoVendor\phpseclib3\Exception\UnsupportedAlgorithmException;
use LassoVendor\phpseclib3\File\ASN1\Element;
use LassoVendor\phpseclib3\File\ASN1\Maps;
use LassoVendor\phpseclib3\Math\BigInteger;
/**
 * Pure-PHP X.509 Parser
 *
 * @author  Jim Wigginton <terrafrost@php.net>
    ...
    private function validateSignatureCountable($caonly, $count)
    {
        ...
        switch (\true) {
            case isset($this->currentCert['tbsCertificate']):
                // self-signed cert
                switch (\true) {
                    case !\defined('LassoVendor\\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertificate']['issuer'] === $this->currentCert['tbsCertificate']['subject']:
                    case \defined('LassoVendor\\FILE_X509_IGNORE_TYPE') && $this->getIssuerDN(self::DN_STRING) === $this->getDN(self::DN_STRING):
                ...
                if (!empty($this->CAs)) {
                    for ($i = 0; $i < \count($this->CAs); $i++) {
                        ...
                        $ca = $this->CAs[$i];
                        switch (\true) {
                            case !\defined('LassoVendor\\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertificate']['issuer'] === $ca['tbsCertificate']['subject']:
                            case \defined('LassoVendor\\FILE_X509_IGNORE_TYPE') && $this->getDN(self::DN_STRING, $this->currentCert['tbsCertificate']['issuer']) === $this->getDN(self::DN_STRING, $ca['tbsCertificate']['subject']):
                                $authorityKey = $this->getExtension('id-ce-authorityKeyIdentifier');
            ...
            case isset($this->currentCert['tbsCertList']):
                if (!empty($this->CAs)) {
                    for ($i = 0; $i < \count($this->CAs); $i++) {
                        $ca = $this->CAs[$i];
                        switch (\true) {
                            case !\defined('LassoVendor\\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertList']['issuer'] === $ca['tbsCertificate']['subject']:
                            case \defined('LassoVendor\\FILE_X509_IGNORE_TYPE') && $this->getDN(self::DN_STRING, $this->currentCert['tbsCertList']['issuer']) === $this->getDN(self::DN_STRING, $ca['tbsCertificate']['subject']):
                                $authorityKey = $this->getExtension('id-ce-authorityKeyIdentifier');
                                $subjectKeyID = $this->getExtension('id-ce-subjectKeyIdentifier', $ca);
                                switch (\true) {
                                    case !\is_array($authorityKey):
