...
class SdkClientEntity
{
    ...
    public function __construct($params)
    {
        ...
        $stream->setFormatter(new Monolog\Formatter\LineFormatter("[%datetime%] %channel%.%level_name%: [{$tag}] %message%\n"));
    ...
    public static function createSdkClient($params, $logger)
    {
        ...
        $options = ['event_publisher' => LaunchDarkly\Integrations\Guzzle::eventPublisher(), 'logger' => $logger];
    ...
    {
        $this->_client->alias($this->makeUser($params['user']), $this->makeUser($params['previousUser']));
    }
    private function doCustomEvent($params)
    {
        $this->_client->track($params['eventKey'], $this->makeUser($params['user']), $params['data'] ?? null, $params['metricValue'] ?? null);
    }
    private function doEvaluate($params)
    {
        ...
        if ($detail) {
            ...
            return ["value" => $result->getValue(), "variationIndex" => $result->getVariationIndex(), "reason" => $result->getReason()];
        } else {
    ...
    private function doEvaluateAll($params)
    {
        ...
        return ['state' => $state->jsonSerialize()];
    }
