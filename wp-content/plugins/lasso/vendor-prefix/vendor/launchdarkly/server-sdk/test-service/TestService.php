...
class TestService
{
    ...
    public function __construct($store, $logger)
    {
        ...
        $this->_app->set('flight.log_errors', \true);
    ...
    {
        return ['name' => 'php-server-sdk', 'capabilities' => ['php', 'server-side', 'all-flags-client-side-only', 'all-flags-details-only-for-tracked-flags', 'all-flags-with-reasons', 'secure-mode-hash'], 'clientVersion' => \LassoVendor\LaunchDarkly\LDClient::VERSION];
    }
    public function createClient($params)
    {
        $this->_logger->info("Creating client with parameters: " . \json_encode($params));
        $client = new SdkClientEntity($params, $this->_logger);
        // just to verify that the config is valid
