...
namespace LassoVendor\LaunchDarkly;

use LassoVendor\LaunchDarkly\Impl\Events\EventFactory;
use LassoVendor\LaunchDarkly\Impl\Events\EventProcessor;
use LassoVendor\LaunchDarkly\Impl\Events\NullEventProcessor;
use LassoVendor\LaunchDarkly\Impl\Model\FeatureFlag;
use LassoVendor\LaunchDarkly\Impl\PreloadedFeatureRequester;
use LassoVendor\LaunchDarkly\Impl\UnrecoverableHTTPStatusException;
use LassoVendor\LaunchDarkly\Integrations\Guzzle;
use LassoVendor\LaunchDarkly\Types\ApplicationInfo;
use LassoVendor\Monolog\Handler\ErrorLogHandler;
...
class LDClient
{
    ...
    public function __construct(string $sdkKey, array $options = [])
    {
        ...
        if (isset($options['offline']) && $options['offline'] === \true) {
            $this->_offline = \true;
        ...
        $this->_eventFactoryWithReasons = new EventFactory(\true);
    ...
    private function getFeatureRequester(string $sdkKey, array $options) : FeatureRequester
    {
         ...
         */
        if (\is_a($fr, FeatureRequester::class, \true)) {
    ...
    public function allFlagsState(LDUser $user, array $options = []) : FeatureFlagsState
    {
        ...
        $state = new FeatureFlagsState(\true);
