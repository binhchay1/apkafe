...
class FeatureFlagsState implements \JsonSerializable
{
     ...
     * @return void
     */
    public function addFlag(FeatureFlag $flag, EvaluationDetail $detail, bool $withReason = \false, bool $detailsOnlyIfTracked = \false) : void
    {
        ...
        if ($trackEvents) {
            $meta['trackEvents'] = \true;
        }
        if ($trackReason) {
            $meta['trackReason'] = \true;
        }
    ...
    public function jsonSerialize() : array
    {
        ...
        if (\count($this->_flagMetadata) === 0) {
            $metaMap = new \stdClass();
            // using object rather than array ensures the JSON value is {}, not []
        } else {
            $metaMap = [];
