...
namespace LassoVendor\LaunchDarkly\Integrations\TestData;

\define('LassoVendor\\TRUE_VARIATION_INDEX', 0);
\define('LassoVendor\\FALSE_VARIATION_INDEX', 1);
/**
...
class FlagBuilder
{
    ...
    {
        return \count($this->_variations) === 2 && $this->_variations[TRUE_VARIATION_INDEX] === \true && $this->_variations[FALSE_VARIATION_INDEX] === \false;
    }
    /**
    ...
    public function booleanFlag()
    {
        if ($this->_isBooleanFlag()) {
            ...
            return $this->variations(\true, \false)->fallthroughVariation(TRUE_VARIATION_INDEX)->offVariation(FALSE_VARIATION_INDEX);
        }
    }
    /**
    ...
    public function variationForAllUsers($variation)
    {
        if (\is_bool($variation)) {
            ...
            return $this->on(\true)->clearRules()->clearUserTargets()->fallthroughVariation($variation);
        }
    }
    /**
    ...
    {
        $json = \json_decode(\json_encode($value), \true);
    ...
    {
        if (\is_bool($variation)) {
            return $this->booleanFlag()->variationForUser($userKey, $this->variationForBoolean($variation));
        } else {
    ...
    public function build(int $version) : array
    {
        ...
        $baseFlagObject['fallthrough'] = ['variation' => $this->_fallthroughVariation];
        $targets = [];
        foreach ($this->_targets as $varIndex => $userKeys) {
            $targets[$varIndex] = ['variation' => $varIndex, 'values' => $userKeys];
        }
        // used to reset index
        \ksort($targets);
