...
use LassoVendor\LaunchDarkly\Impl\Model\FeatureFlag;
...
class EventFactory
{
     ...
     * @return (mixed|null)[]
     */
    public function newEvalEvent(FeatureFlag $flag, LDUser $user, EvaluationDetail $detail, $default, $prereqOfFlag = null) : array
    {
        $addExperimentData = $flag->isExperiment($detail->getReason());
        $e = ['kind' => 'feature', 'creationDate' => Util::currentTimeUnixMillis(), 'key' => $flag->getKey(), 'user' => $user, 'variation' => $detail->getVariationIndex(), 'value' => $detail->getValue(), 'default' => $default, 'version' => $flag->getVersion()];
    ...
    {
        $e = ['kind' => 'feature', 'creationDate' => Util::currentTimeUnixMillis(), 'key' => $flag->getKey(), 'user' => $user, 'value' => $detail->getValue(), 'default' => $detail->getValue(), 'version' => $flag->getVersion()];
        // the following properties are handled separately so we don't waste bandwidth on unused keys
    ...
    {
        $e = ['kind' => 'feature', 'creationDate' => Util::currentTimeUnixMillis(), 'key' => $key, 'user' => $user, 'value' => $detail->getValue(), 'default' => $detail->getValue()];
        // the following properties are handled separately so we don't waste bandwidth on unused keys
        if ($this->_withReasons) {
            $e['reason'] = $detail->getReason()->jsonSerialize();
        }
    ...
    {
        return ['kind' => 'identify', 'creationDate' => Util::currentTimeUnixMillis(), 'key' => \strval($user->getKey()), 'user' => $user];
    }
    /**
    ...
    {
        $e = ['kind' => 'custom', 'creationDate' => Util::currentTimeUnixMillis(), 'key' => $eventName, 'user' => $user];
        if (isset($data)) {
            $e['data'] = $data;
        }
    ...
    {
        $e = ['kind' => 'alias', 'key' => \strval($user->getKey()), 'contextKind' => static::contextKind($user), 'previousKey' => \strval($previousUser->getKey()), 'previousContextKind' => static::contextKind($previousUser), 'creationDate' => Util::currentTimeUnixMillis()];
        return $e;
    }
