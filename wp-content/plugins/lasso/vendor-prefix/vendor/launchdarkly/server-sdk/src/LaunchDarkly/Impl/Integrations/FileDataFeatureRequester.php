    ...
    private function loadFile(string $filePath, array &$flags, array &$segments) : void
    {
        ...
        if (isset($data['flagValues'])) {
            foreach ($data['flagValues'] as $key => $value) {
                $flag = FeatureFlag::decode(["key" => $key, "version" => 1, "on" => \false, "prerequisites" => [], "salt" => "", "targets" => [], "rules" => [], "fallthrough" => [], "offVariation" => 0, "variations" => [$value], "deleted" => \false, "trackEvents" => \false, "clientSide" => \false]);
                $this->tryToAdd($flags, $key, $flag, "feature flag");
            }
        }
