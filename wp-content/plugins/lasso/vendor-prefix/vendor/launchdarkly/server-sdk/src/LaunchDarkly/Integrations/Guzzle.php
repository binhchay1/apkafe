...
namespace LassoVendor\LaunchDarkly\Integrations;
...
class Guzzle
{
    ...
    public static function featureRequester($options = [])
    {
        return function (string $baseUri, string $sdkKey, array $baseOptions) use($options) {
            return new \LassoVendor\LaunchDarkly\Impl\Integrations\GuzzleFeatureRequester($baseUri, $sdkKey, \array_merge($baseOptions, $options));
        };
    }
    /**
    ...
    public static function eventPublisher($options = [])
    {
        return function (string $sdkKey, array $baseOptions) use($options) {
            return new \LassoVendor\LaunchDarkly\Impl\Integrations\GuzzleEventPublisher($sdkKey, \array_merge($baseOptions, $options));
        };
    }
}
