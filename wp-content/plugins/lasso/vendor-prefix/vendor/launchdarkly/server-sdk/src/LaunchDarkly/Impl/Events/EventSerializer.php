...
class EventSerializer
{
    ...
    private function filterAttrs(array $attrs, array &$json, ?array $userPrivateAttrs, array &$allPrivateAttrs, bool $stringify) : void
    {
        foreach ($attrs as $key => $value) {
            if ($value !== null) {
                if ($this->_allAttrsPrivate || !\is_null($userPrivateAttrs) && \in_array($key, $userPrivateAttrs) || \in_array($key, $this->_privateAttrNames)) {
                    $allPrivateAttrs[] = $key;
                } else {
    ...
    private function serializeUser(LDUser $user) : array
    {
        ...
        $allPrivateAttrs = [];
        $attrs = ['secondary' => $user->getSecondary(), 'ip' => $user->getIP(), 'country' => $user->getCountry(), 'email' => $user->getEmail(), 'name' => $user->getName(), 'avatar' => $user->getAvatar(), 'firstName' => $user->getFirstName(), 'lastName' => $user->getLastName()];
        $this->filterAttrs($attrs, $json, $userPrivateAttrs, $allPrivateAttrs, \true);
        ...
        if (!\is_null($custom) && !empty($user->getCustom())) {
            ...
            if ($customOut) {
                // if this is empty, we will return a json array for 'custom' instead of an object
