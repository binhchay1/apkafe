...
use LassoVendor\GuzzleHttp\Exception\BadResponseException;
use LassoVendor\GuzzleHttp\HandlerStack;
use LassoVendor\Kevinrob\GuzzleCache\CacheMiddleware;
use LassoVendor\Kevinrob\GuzzleCache\Strategy\PublicCacheStrategy;
use LassoVendor\LaunchDarkly\FeatureRequester;
use LassoVendor\LaunchDarkly\Impl\Model\FeatureFlag;
use LassoVendor\LaunchDarkly\Impl\Model\Segment;
...
class GuzzleFeatureRequester implements FeatureRequester
{
    ...
    {
        $baseUri = \LassoVendor\LaunchDarkly\Impl\Util::adjustBaseUri($baseUri);
        $this->_logger = $options['logger'];
        $stack = HandlerStack::create();
        if (\class_exists('LassoVendor\\Kevinrob\\GuzzleCache\\CacheMiddleware')) {
            $stack->push(new CacheMiddleware(new PublicCacheStrategy($options['cache'] ?? null)), 'cache');
            ...
            $this->_loggedCacheNotice = \true;
        }
        $defaults = ['headers' => Util::defaultHeaders($sdkKey, $options['application_info'] ?? null), 'timeout' => $options['timeout'], 'connect_timeout' => $options['connect_timeout'], 'handler' => $stack, 'debug' => $options['debug'] ?? \false, 'base_uri' => $baseUri];
    ...
    public function getFeature(string $key) : ?FeatureFlag
    {
        try {
            ...
            return FeatureFlag::decode(\json_decode($body->getContents(), \true));
    ...
    public function getSegment(string $key) : ?Segment
    {
        try {
            ...
            $body = $response->getBody();
            return Segment::decode(\json_decode($body->getContents(), \true));
        } catch (BadResponseException $e) {
            /** @psalm-suppress PossiblyNullReference (resolved in guzzle 7) */
            $code = $e->getResponse()->getStatusCode();
            if ($code == 404) {
    ...
    public function getAllFeatures() : ?array
    {
        try {
            ...
            $body = $response->getBody();
            return \array_map(FeatureFlag::getDecoder(), \json_decode($body->getContents(), \true));
        } catch (BadResponseException $e) {
            /** @psalm-suppress PossiblyNullReference (resolved in guzzle 7) */
