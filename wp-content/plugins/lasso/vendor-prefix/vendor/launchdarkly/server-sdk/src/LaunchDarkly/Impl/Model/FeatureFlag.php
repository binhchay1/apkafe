...
namespace LassoVendor\LaunchDarkly\Impl\Model;

use LassoVendor\LaunchDarkly\EvaluationDetail;
use LassoVendor\LaunchDarkly\EvaluationReason;
use LassoVendor\LaunchDarkly\FeatureRequester;
use LassoVendor\LaunchDarkly\Impl\EvalResult;
use LassoVendor\LaunchDarkly\Impl\Events\EventFactory;
...
class FeatureFlag
{
    ...
    protected function __construct(string $key, int $version, bool $on, array $prerequisites, ?string $salt, array $targets, array $rules, VariationOrRollout $fallthrough, ?int $offVariation, array $variations, bool $deleted, bool $trackEvents, bool $trackEventsFallthrough, ?int $debugEventsUntilDate, bool $clientSide)
    {
        $this->_key = $key;
    ...
    {
        return function ($v) {
            return new FeatureFlag($v['key'], $v['version'], $v['on'], \array_map(Prerequisite::getDecoder(), $v['prerequisites'] ?: []), $v['salt'], \array_map(Target::getDecoder(), $v['targets'] ?: []), \array_map(Rule::getDecoder(), $v['rules'] ?: []), \call_user_func(VariationOrRollout::getDecoder(), $v['fallthrough']), $v['offVariation'], $v['variations'] ?: [], $v['deleted'], isset($v['trackEvents']) && $v['trackEvents'], isset($v['trackEventsFallthrough']) && $v['trackEventsFallthrough'], isset($v['debugEventsUntilDate']) ? $v['debugEventsUntilDate'] : null, isset($v['clientSide']) && $v['clientSide']);
        };
    }
    ...
    private function evaluateInternal(LDUser $user, FeatureRequester $featureRequester, array &$events, EventFactory $eventFactory) : EvaluationDetail
    {
        if (!$this->isOn()) {
    ...
    private function getValueForVariationOrRollout(VariationOrRollout $r, LDUser $user, EvaluationReason $reason) : EvaluationDetail
    {
        ...
        if ($inExperiment) {
            if ($reason->getKind() === EvaluationReason::FALLTHROUGH) {
                $reason = EvaluationReason::fallthrough(\true);
            } elseif ($reason->getKind() === EvaluationReason::RULE_MATCH) {
                $reason = EvaluationReason::ruleMatch($reason->getRuleIndex(), $reason->getRuleId(), \true);
            }
        }
