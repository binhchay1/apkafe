...
namespace LassoVendor\GuzzleHttp\Handler;

use LassoVendor\GuzzleHttp\Exception\ConnectException;
use LassoVendor\GuzzleHttp\Exception\RequestException;
use LassoVendor\GuzzleHttp\Promise as P;
use LassoVendor\GuzzleHttp\Promise\FulfilledPromise;
use LassoVendor\GuzzleHttp\Promise\PromiseInterface;
use LassoVendor\GuzzleHttp\Psr7\LazyOpenStream;
use LassoVendor\GuzzleHttp\TransferStats;
use LassoVendor\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \LassoVendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \LassoVendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
