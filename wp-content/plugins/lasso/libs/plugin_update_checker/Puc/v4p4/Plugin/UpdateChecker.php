...
if ( !class_exists('Lasso_Puc_v4p4_Plugin_UpdateChecker', false) ):
	 ...
	 * @access public
	 */
	class Lasso_Puc_v4p4_Plugin_UpdateChecker extends Lasso_Puc_v4p4_UpdateChecker {
		...
		/**
		 * @var Lasso_Puc_v4p4_Plugin_Package
		 */
		protected $package;
		...
		public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
			...
			$this->extraUi = new Lasso_Puc_v4p4_Plugin_Ui($this);
		}
		...
		protected function createScheduler($checkPeriod) {
			$scheduler = new Lasso_Puc_v4p4_Scheduler($this, $checkPeriod, array('load-plugins.php'));
		 ...
		 * @return Lasso_Puc_v4p4_Plugin_Info
		 */
		public function requestInfo($queryArgs = array()) {
			list($pluginInfo, $result, $queryArgs) = $this->requestMetadata('Lasso_Puc_v4p4_Plugin_Info', 'request_info', $queryArgs);

			if ( $pluginInfo !== null ) {
				/** @var Lasso_Puc_v4p4_Plugin_Info $pluginInfo */
				$pluginInfo->filename = $this->pluginFile;
				$pluginInfo->slug = $this->slug;
			}

			$checking_for_updates = isset($queryArgs['checking_for_updates']) ? $queryArgs['checking_for_updates'] : 0;
			
			if(is_object($pluginInfo)) { 
				$pluginInfo->php = $queryArgs['php']; 
				$pluginInfo->locale = $queryArgs['locale']; 
				$pluginInfo->checking_for_updates = $checking_for_updates; 
				$pluginInfo->datetime = $queryArgs['datetime']; 
				$pluginInfo->wordpress_version = $queryArgs['wordpress_version']; 
				$pluginInfo->mysql_version = $queryArgs['mysql_version']; 
				$pluginInfo->license_key = $queryArgs['license_key']; 
				$pluginInfo->install_url = $queryArgs['install_url']; 
				$pluginInfo->is_classic_editor = $queryArgs['is_classic_editor']; 
				
				$pluginInfo->external = $pluginInfo->homepage;

				// Add update detail
				$pluginInfo->banners = [
					'low' => LASSO_PLUGIN_URL . 'admin/assets/images/lasso-plugin-banner-722x250.jpg',
					'high' => LASSO_PLUGIN_URL . 'admin/assets/images/lasso-plugin-banner-1544x500.jpg'
				];

				$pluginInfo->icons = [
					'default' => LASSO_PLUGIN_URL . 'admin/assets/images/lasso-256x256.png'
				];

				// Changelog tab
				$changelog = '';
				if(isset($pluginInfo->releases) && count($pluginInfo->releases) > 0) {
					foreach($pluginInfo->releases as $release) {
						$changelog .= '<h4>' . $release->version . '</h4>';
						$changelog .= str_replace("\r\n", '<br>', $release->changelog);
					}
				}

				$pluginInfo->sections = [
					'description' => $pluginInfo->repo->description ?? '',
					'changelog' => $changelog
				];

				$site_id = $pluginInfo->site_id ?? '';
				if(isset($site_id) && $site_id != '') {
    				Lasso_License::save_site_id($site_id); // save site_id from DB
				}
			} 
			// reactivate license
			...
			// show plugin details in update popup
			if($pluginInfo != null) {
				update_option('lasso_old_update', $pluginInfo, false);
			} else if(!Lasso_License::get_license_status()) {
				$old_update = get_option('lasso_old_update', null);
				if($old_update != null) {
					$old_update->download_url = '';
					$pluginInfo = $old_update;
				}
			}
		 ...
		 *
		 * @return Lasso_Puc_v4p4_Update|null An instance of Plugin_Update, or NULL when no updates are available.
		 */
		public function requestUpdate() {
			Lasso_Helper::write_log( 'Check new version', 'lasso_update_checker' );
			...
			$update = Lasso_Puc_v4p4_Plugin_Update::fromPluginInfo($pluginInfo);
		 ...
		 *
		 * @return Lasso_Puc_v4p4_Plugin_Update|null
		 */
		public function getUpdate() {
			$update = parent::getUpdate();
			if ( isset($update) ) {
				/** @var Lasso_Puc_v4p4_Plugin_Update $update */
		...
		protected function createDebugBarExtension() {
			return new Lasso_Puc_v4p4_DebugBar_PluginExtension($this);
		}
		...
		protected function createInstalledPackage() {
			return new Lasso_Puc_v4p4_Plugin_Package($this->pluginAbsolutePath, $this);
		}

		/**
		 * @return Lasso_Puc_v4p4_Plugin_Package
		 */
