...
if ( !class_exists('Lasso_Puc_v4p4_StateStore', false) ):

	class Lasso_Puc_v4p4_StateStore {
		/**
		...
		/**
		 * @var Lasso_Puc_v4p4_Update|null Cached update.
		 */
		...
		/**
		 * @return null|Lasso_Puc_v4p4_Update
		 */
		 ...
		 * @return $this
		 */
		public function setUpdate(Lasso_Puc_v4p4_Update $update = null) {
			$this->lazyLoad();
			$this->update = $update;
			return $this;
		}
		...
		public function save() {
			...
			if ( isset($this->update)) {
				...
				if ( Lasso_Puc_v4p4_Utils::startsWith($updateClass, $prefix) ) {
		...
		protected function load() {
			...
			$this->lastCheck = intval(Lasso_Puc_v4p4_Utils::get($state, 'lastCheck', 0));
			$this->checkedVersion = Lasso_Puc_v4p4_Utils::get($state, 'checkedVersion', '');
			$this->update = null;
