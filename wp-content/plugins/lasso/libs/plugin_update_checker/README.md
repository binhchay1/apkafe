...
4. Add the following code to the main plugin file or to the `functions.php` file:
	...
	$myUpdateChecker = Lasso_Puc_v4_Factory::buildUpdateChecker(
...
2. Add the following code to the main plugin file or `functions.php`:
	...
	$myUpdateChecker = new Lasso_Puc_v4p4_Vcs_PluginUpdateChecker(
		new Lasso_Puc_v4p4_Vcs_GitLabApi('https://myserver.com/user-name/repo-name/'),
		__FILE__,
		'unique-plugin-or-theme-slug'
	);
